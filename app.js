...
// Menghubungkan wallet menggunakan MetaMask
async function connectWallet() {
    if (window.ethereum) {
        try {
            ...
            // Menginisialisasi kontrak dengan ABI dan alamatnya
...
// Fungsi untuk menambah jumlah mint
function increaseMint() {
    if (mintAmount < getMaxMintPerWallet() - getWalletMints()) {
        mintAmount++;
...
// Mendapatkan tahap minting saat ini dari kontrak
async function updateMintStage() {
    const stage = await contract.methods.currentStage().call();
    let stageInfo = '';
    
    switch (stage) {
        case '0': // Closed
            stageInfo = 'Minting Closed';
            document.getElementById('mintButton').disabled = true;
            break;
        case '1': // Free Mint
            stageInfo = 'Free Mint';
            document.getElementById('mintPrice').innerText = 'Price: Free';
            document.getElementById('mintButton').disabled = false;
            break;
        case '2': // Presale
            stageInfo = 'Presale';
            document.getElementById('mintPrice').innerText = 'Price: 0.0005 ETH per NFT';
            document.getElementById('mintButton').disabled = false;
            break;
        case '3': // Public Sale
            stageInfo = 'Public Sale';
            document.getElementById('mintPrice').innerText = 'Price: 0.0015 ETH per NFT';
            document.getElementById('mintButton').disabled = false;
            break;
        default:
            stageInfo = 'Unknown Stage';
    }
    
    document.getElementById('stageInfo').innerText = `Current Stage: ${stageInfo}`;
}

// Mendapatkan batas maksimum minting per wallet berdasarkan tahap minting
function getMaxMintPerWallet() {
    const stage = document.getElementById('stageInfo').innerText;
    if (stage.includes("Free Mint")) return 1;
    if (stage.includes("Presale")) return 2;
    if (stage.includes("Public Sale")) return 5;
    return 0;
}

// Mendapatkan jumlah NFT yang sudah di-mint oleh pengguna dari kontrak
async function getWalletMints() {
    return await contract.methods.walletMints(userAccount).call();
}
...
async function mintNFT() {
    ...
    if (stage.includes("Presale")) {
        price = web3.utils.toWei("0.0005", "ether");
    } else if (stage.includes("Public Sale")) {
        price = web3.utils.toWei("0.0015", "ether");
    }

    try {
        // Kirim transaksi mint ke kontrak
        ...
        updateMintStage(); // Update tahap minting setelah mint
    } catch (error) {
