...
const contractAddress = "YOUR_CONTRACT_ADDRESS"; // Ganti dengan alamat kontrak
const contractABI = [
...
async function connectWallet() {
    if (window.ethereum) {
        try {
            // Meminta akses dompet pengguna
            await window.ethereum.request({ method: 'eth_requestAccounts' });
            provider = new ethers.providers.Web3Provider(window.ethereum);
            signer = provider.getSigner();
            userAddress = await signer.getAddress();

            // Tampilkan informasi dompet dan tombol Mint NFT
            displayWalletInfo();
            await updateWalletBalance(); // Perbarui saldo pengguna
        } catch (error) {
...
// Menampilkan informasi dompet setelah koneksi berhasil
function displayWalletInfo() {
    // Sembunyikan tombol Connect Wallet, tampilkan tombol Mint NFT dan info dompet
...
async function mintNFT() {
    try {
        ...
        // Perbarui saldo setelah minting
        await updateWalletBalance();
        updateSupplyTracker();
    } catch (error) {
        console.error("Minting gagal:", error);
        alert("Minting gagal. Cek saldo atau coba lagi nanti.");
    }
}
...
function disconnectWallet() {
    ...
    // Tampilkan kembali tombol Connect Wallet, sembunyikan info dompet dan tombol Mint NFT
